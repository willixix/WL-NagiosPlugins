...
my %conf = (
    TIMEOUT            => 15,
    CFG_DIR            => "/opt/nagios/addons/pnp4nagios/etc/",
    USE_RRDs           => 1,
    RRDPATH            => "/opt/nagios/addons/pnp4nagios/var/perfdata",
    RRDTOOL            => "/usr/bin/rrdtool",
    RRD_STORAGE_TYPE   => "SINGLE",
    RRD_HEARTBEAT      => 8640,
    RRA_STEP           => 60,
    RRA_CFG            => "/opt/nagios/addons/pnp4nagios/etc/rra.cfg",
    STATS_DIR          => "/opt/nagios/addons/pnp4nagios/var/stats",
    LOG_FILE           => "/opt/nagios/addons/pnp4nagios/var/log/perfdata.log",
...
sub read_custom_template {
    ...
    if ( -e $template_cfg ) {
        ...
        while (<FH>) {
		    ...
		    if (scalar(@dstype_list) > 1) {
			$dstype = $initial_dstype;
			for (my $i=0; $i<scalar(@dstype_list);$i++) {
			    if ($dstype_list[$i] =~ /^(COUNTER|GAUGE|ABSOLUTE|DERIVE)$/ ) {
				 $dslist[$i] = {} if !exists($dslist[$i]);
				 $dslist[$i]{'DSTYPE'}=$dstype_list[$i];
		...
		# New type of per-DS definition added by William L.
		# DS = id:name:datatype:heartbeat:use_min:use_max
		if ( $1 eq "DS" ) {
		    my $dsn;
		    my $dsline = $2;
                    my @setds = split /:/, $dsline;
		    if ($setds[0] !~ /\d+/) {
			print_log( "DS Definition '".$dsline."' has invalid invalid DS number '".$setds[0]."' as defined in $template_cfg", 2 );
		    }
		    else {
			$dsn = $setds[0] - 1;
			if (exists($setds[1])) {
			    $dslist[$dsn]={} if !exists($dslist[$dsn]);
			    $dslist[$dsn]{'NAME'} = $setds[1];
			}
			if (exists($setds[2])) {
			    if ($setds[2] =~ /^(COUNTER|GAUGE|ABSOLUTE|DERIVE)$/ ) {
				 $dslist[$dsn]{'DSTYPE'}=$setds[2];
				 print_log( "Adapting RRD Datatype to '".$setds[2]."' as defined in $template_cfg with key $dsn for '".$setds[1]."'", 2 );
			    }
			    else {
				  print_log( "DS Definition '".$dsline."'has invalid RRD Datatype'".$setds[2]."' as defined in $template_cfg", 2 );
			    }
			    $dslist[$dsn]{'RRD_HEARTBEAT'} = $setds[3] if exists($setds[3]);
			    $dslist[$dsn]{'USE_MIN_ON_CREATE'} = 1 if exists($setds[4]) && $setds[4] eq "1";
			    $dslist[$dsn]{'USE_MAX_ON_CREATE'} = 1 if exists($setds[5]) && $setds[5] eq "1";
			}
		    }
		}
		if ( $1 eq "ONLY_NAMED_DS"&& $2 eq "1" ) {
		    $only_named_ds = 1;
		}
            }
        }
    ...
    if (scalar(@dslist)>0) {
	for (my $i=0; $i<scalar(@dslist);$i++) {
	    $dslist[$i] = {} if !exists($dslist[$i]);
	    $dslist[$i]{'DSTYPE'} = $dstype if !exists($dslist[$i]{'DSTYPE'});
	    $dslist[$i]{'RRD_HEARTBEAT'} = $rrd_heartbeat if !exists($dslist[$i]{'RRD_HEARTBEAT'});
	    $dslist[$i]{'USE_MIN_ON_CREATE'} = $use_min_on_create if !exists($dslist[$i]{'USE_MIN_ON_CREATE'});
	    $dslist[$i]{'USE_MAX_ON_CREATE'} = $use_max_on_create if !exists($dslist[$i]{'USE_MAX_ON_CREATE'});
	    $dsnames->{ $dslist[$i]{'NAME'} } = $i if exists($dslist[$i]{'NAME'}); # hash lookup table from perf names to ids
	}
    }

    $CTPL->{TEMPLATE}		= $template;
    $CTPL->{DEFAULT_DSTYPE}	= $initial_dstype;
    $CTPL->{SET_DSTYPE}		= $dstype;
    $CTPL->{DSTYPE}       	= $dstype;
    ...
    $CTPL->{DSLIST}		= \@dslist;
    $CTPL->{DSNAMES}		= $dsnames;
    $CTPL->{ONLY_NAMED_DS}	= $only_named_ds;

    print_log("--  TEMPLATE = ". $CTPL->{TEMPLATE}, 3);
    print_log("--  DEFAULT_DSTYPE = ". $CTPL->{DEFAULT_DSTYPE}, 3);
    print_log("--  SET_DSTYPE = ". $CTPL->{SET_DSTYPE}, 3);
    print_log("--  DSTYPE = ". $CTPL->{DSTYPE}, 3);
    print_log("--  RRD_STORAGE_TYPE = ". $CTPL->{RRD_STORAGE_TYPE}, 3);
    print_log("--  RRD_HEARTBEAT = ". $CTPL->{RRD_HEARTBEAT}, 3);
    print_log("--  USE_MIN_ON_CREATE = ". $CTPL->{USE_MIN_ON_CREATE}, 3);
    print_log("--  USE_MAX_ON_CREATE = ". $CTPL->{USE_MAX_ON_CREATE}, 3);
    print_log("--  ONLY_NAMED_DS = ". $CTPL->{ONLY_NAMED_DS}, 3);
    print_log("--  DSLIST = ", 3);
	...
	print_log("  -- " . $ds . " = " . $CTPL->{DSNAMES}{$ds}, 3);
    }
}
...
sub _parse {
    ...
    if ( $tmp_string eq $string ) {
        # normally debug level is 2
        print_log( "No pattern match in function _parse($string)", 1 );
        return undef;
    }
    ...
    my $label = $1;
    my $full_value = $2;
    my $tmp_value = $full_value;

    $full_value =~ s/([\d\.\-]+)([\w\/%]*);?([\d\.\-:~@]+)?;?([\d\.\-:~@]+)?;?([\d\.\-]+)?;?([\d\.\-]+)?;?\s*//;

    # If full_value is non-numeric, ignore it and recursive call if anything else is ok
    if ($tmp_value eq $full_value) {
    ...
    return undef unless ( defined $1 && $1 ne "" );
...
sub parse_perfstring {
    ...
    if ( $perfstring =~ /^[']?([a-zA-Z0-9\.\-_\s\/\#]+)::([a-zA-Z0-9\.\-_\s]+)::([^=]+)[']?=/ ) {
		...
		$perfs[$p{dsid}] = {%p}; # this makes a copy of %p hash array to put into perfs array under the dsid $p{dsid}
                # [WL: above used to be] push @perfs, {%p};
                $count++;
            }
